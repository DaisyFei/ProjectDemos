...
jsPsych.plugins['survey-multi-choice-other'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    display_element.append($('<form>', {
      "id": trial_form_id,
      "style": "position: fixed; width: 43vw; left: 50%; transform: translate(-50%, 0%);"
    }));
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      for (var j = 0; j < trial.questions[0]["options"].length; j++) {
        ...
        var option_label = '<label class="' + plugin_id_name + '-text">' + trial.questions[0]["options"][j] + '</label>';
        $(option_id_selector).append(option_label);

        // create radio button
        var input_id_name = _join(plugin_id_name, 'response', i);
        $(option_id_selector + " label").prepend('<input type="radio" name="' + input_id_name + '" value="' + trial.questions[0]["options"][j] + '">');
      }
      ...
      // add label and question text
      var option_label = '<label class="' + 'other-label"> </label>';
      $('#other_option').append(option_label);
      //create other radio button
      $('#other_option' + " label").prepend('<input type="radio" id="other_opt" name="' + input_id_name + '" value="">  Other: <input type="text" name="other_reason" id="other_text">');
        document.getElementById("other_text").addEventListener('focus', function(){
        document.getElementById("other_opt").click();
      }, true);
    ...
    $trial_form.submit(function(event) {
      ...
      document.getElementById("other_opt").value = document.getElementById("other_text").value;
      console.log($('#other_opt').val());
